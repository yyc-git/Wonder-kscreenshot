...
export default function colorBoard (me) {
    let colorBoard = window.wonder_containerDocument.createElement('span')
    colorBoard.id = 'kssColorBoard'
    colorBoard.title = '颜色板'
    colorBoard.tabIndex = '-1'

    let currentColor = window.wonder_containerDocument.createElement('span')
    ...
    let colorItemWrapper = window.wonder_containerDocument.createElement('div')
    colorItemWrapper.id = 'kssColorItemWrapper'

    colorList.forEach(function (it, index) {
        let dom = window.wonder_containerDocument.createElement('span')
